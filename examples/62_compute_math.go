...
func main() {
	stack_nums := ual.NewStack(ual.LIFO, ual.TypeFloat64)
	stack_nums.Push(floatToBytes(2.000000))
	func() {
		stack_nums.Lock()
		defer stack_nums.Unlock()
		_bytes_x, _err_x := stack_nums.PopRaw()
		if _err_x != nil { panic(_err_x) }
		var x float64 = bytesToFloat(_bytes_x)
		var root float64 = math.Sqrt(x)
		var squared float64 = math.Pow(root, 2.0)
		var diff float64 = math.Abs((squared - x))
		stack_nums.PushRaw(floatToBytes(diff))
		return
	}()
	{ v, _ := stack_nums.Pop(); fmt.Println(bytesToFloat(v)) }
