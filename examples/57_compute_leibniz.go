...
func main() {
	stack_leibniz := ual.NewStack(ual.LIFO, ual.TypeFloat64)
	stack_leibniz.Push(floatToBytes(100000.000000))
	func() {
		stack_leibniz.Lock()
		defer stack_leibniz.Unlock()
		_bytes_terms, _err_terms := stack_leibniz.PopRaw()
		if _err_terms != nil { panic(_err_terms) }
		var terms float64 = bytesToFloat(_bytes_terms)
		var sum float64 = 0.0
		...
		var pi float64 = (4.0 * sum)
		stack_leibniz.PushRaw(floatToBytes(pi))
		return
	}()
	stack_leibniz.Push(floatToBytes(3.141590))
	func() {
		stack_leibniz.Lock()
		defer stack_leibniz.Unlock()
		_bytes_expected, _err_expected := stack_leibniz.PopRaw()
		if _err_expected != nil { panic(_err_expected) }
		var expected float64 = bytesToFloat(_bytes_expected)
		_bytes_actual, _err_actual := stack_leibniz.PopRaw()
		...
		if (diff < 0.001) {
			stack_leibniz.PushRaw(floatToBytes(1.0))
			return
		}
		stack_leibniz.PushRaw(floatToBytes(0.0))
		return
	}()
