...
func main() {
	stack_vec := ual.NewStack(ual.Indexed, ual.TypeInt64)
	stack_vec.Push(intToBytes(0))
	stack_vec.Push(intToBytes(0))
	stack_vec.Push(intToBytes(0))
	stack_vec.Push(intToBytes(0))
	stack_vec.Push(intToBytes(0))
	...
	func() {
		stack_vec.Lock()
		defer stack_vec.Unlock()
		var sum int64 = 0
		var i int64 = 0
		for (i < 10) {
			sum = (sum + func() int64 { _b, _ok := stack_vec.GetAtRaw(int(i)); if !_ok { panic("compute: index out of bounds") }; return bytesToInt(_b) }())
			i = (i + 1)
		}
		stack_vec.PushRaw(intToBytes(sum))
		return
	}()
	{ v, _ := stack_vec.Pop(); fmt.Println(bytesToInt(v)) }
	stack_vec.Push(intToBytes(5))
	func() {
		stack_vec.Lock()
		defer stack_vec.Unlock()
		_bytes_n, _err_n := stack_vec.PopRaw()
		if _err_n != nil { panic(_err_n) }
		var n int64 = bytesToInt(_bytes_n)
		var squares [10]int64
		var i int64 = 0
		for (i < n) {
			squares[int(i)] = ((i + 1) * (i + 1))
			i = (i + 1)
		}
		var sum int64 = 0
		i = 0
		for (i < n) {
			sum = (sum + squares[int(i)])
			i = (i + 1)
		}
		stack_vec.PushRaw(intToBytes(sum))
		return
