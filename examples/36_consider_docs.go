...
func validate(n int64) int64 {
	stack_i64.PushAt(0, intToBytes(int64(n))) // param n
	if func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }() < int64(0) {
		_consider_status = "negative"
		_consider_value = func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }()
		return 0
	}
	if func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }() > int64(100) {
		_consider_status = "overflow"
		_consider_value = func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }()
		return 100
	}
	_consider_status = "ok"
	return func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }()
}

func safe_sqrt(n int64) int64 {
	stack_i64.PushAt(1, intToBytes(int64(n))) // param n
...
func main() {
	...
	func() {
		...
		stack_i64.PushAt(2, intToBytes(int64(validate(150)))) // var x
		...
		case "negative":
			...
			stack_dstack.Push(intToBytes(val))
			{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(-bytesToInt(v))) }
			{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
		case "overflow":
			var val int64
			switch _v := _consider_value.(type) {
	...
	func() {
		...
		stack_i64.PushAt(3, intToBytes(int64(safe_sqrt(100)))) // var root
		{ v, _ := stack_i64.PeekAt(3); stack_dstack.Push(v) } // push root
