...
func main() {
	...
	stack_fib.Push(intToBytes(20))
	func() {
		stack_fib.Lock()
		defer stack_fib.Unlock()
		_bytes_n, _err_n := stack_fib.PopRaw()
		...
		stack_fib.PushRaw(intToBytes(dp[int(n)]))
		return
	}()
	{ v, _ := stack_fib.Pop(); fmt.Println(bytesToInt(v)) }
