...
func main() {
	stack_config := ual.NewStack(ual.Hash, ual.TypeInt64)
	stack_config.Push(intToBytes(800), []byte("width")) // set "width"
	stack_config.Push(intToBytes(600), []byte("height")) // set "height"
	stack_config.Push(intToBytes(32), []byte("depth")) // set "depth"
	{ v, err := stack_config.Peek([]byte("width")); if err != nil { panic(err) }; stack_dstack.Push(v) } // get "width"
