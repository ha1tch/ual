...
func main() {
	stack_vec := ual.NewStack(ual.Indexed, ual.TypeFloat64)
	stack_vec.Push(floatToBytes(1.000000))
	stack_vec.Push(floatToBytes(2.000000))
	stack_vec.Push(floatToBytes(3.000000))
	func() {
		stack_vec.Lock()
		defer stack_vec.Unlock()
		var sum float64 = 0.0
		var i float64 = float64(0)
		var len float64 = float64(3)
		for (i < len) {
			var val float64 = func() float64 { _b, _ok := stack_vec.GetAtRaw(int(i)); if !_ok { panic("compute: index out of bounds") }; return bytesToFloat(_b) }()
			sum = (sum + (val * val))
			i = (i + float64(1))
		}
		stack_vec.PushRaw(floatToBytes(sum))
		return
	}()
	stack_check := ual.NewStack(ual.LIFO, ual.TypeInt64)
	stack_check.Push(intToBytes(14))
