...
func main() {
	...
	for func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }() < int64(1000) {
		...
		{ v, _ := stack_i64.PeekAt(1); stack_data.Push(v) } // push val
		{ v, _ := stack_i64.PeekAt(0); stack_dstack.Push(v) } // push k
		{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) + 1)) }
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(0, v) } // k = ...
	}
	stack_i64.PushAt(2, intToBytes(int64(0))) // var total
	{ // for @data
		_forLen := stack_data.Len()
		for _forIdx := _forLen - 1; _forIdx >= 0; _forIdx-- {
			_forVal, _ := stack_data.PeekAt(_forIdx)
			stack_i64.PushAt(3, _forVal) // v
			{ v, _ := stack_i64.PeekAt(2); stack_dstack.Push(v) } // push total
			{ v, _ := stack_i64.PeekAt(3); stack_dstack.Push(v) } // push v
			{ b, _ := stack_dstack.Pop(); a, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(a) + bytesToInt(b))) }
			{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(2, v) } // total = ...
		}
	}
	stack_i64.PushAt(4, intToBytes(int64(999999))) // var minimum
	{ // for @data
		_forLen := stack_data.Len()
		for _forIdx := _forLen - 1; _forIdx >= 0; _forIdx-- {
			_forVal, _ := stack_data.PeekAt(_forIdx)
			stack_i64.PushAt(5, _forVal) // v
			if func() int64 { v, _ := stack_i64.PeekAt(5); return bytesToInt(v) }() < func() int64 { v, _ := stack_i64.PeekAt(4); return bytesToInt(v) }() {
				{ v, _ := stack_i64.PeekAt(5); stack_dstack.Push(v) } // push v
				{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(4, v) } // minimum = ...
			}
		}
	}
	stack_i64.PushAt(6, intToBytes(int64(0))) // var maximum
	{ // for @data
		_forLen := stack_data.Len()
		for _forIdx := _forLen - 1; _forIdx >= 0; _forIdx-- {
			_forVal, _ := stack_data.PeekAt(_forIdx)
			stack_i64.PushAt(7, _forVal) // v
			if func() int64 { v, _ := stack_i64.PeekAt(7); return bytesToInt(v) }() > func() int64 { v, _ := stack_i64.PeekAt(6); return bytesToInt(v) }() {
				{ v, _ := stack_i64.PeekAt(7); stack_dstack.Push(v) } // push v
				{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(6, v) } // maximum = ...
			}
		}
	}
	{ v, _ := stack_i64.PeekAt(2); stack_dstack.Push(v) } // push total
	{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
	{ v, _ := stack_i64.PeekAt(4); stack_dstack.Push(v) } // push minimum
	{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
	{ v, _ := stack_i64.PeekAt(6); stack_dstack.Push(v) } // push maximum
	{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
