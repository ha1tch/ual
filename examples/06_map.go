...
func main() {
	...
	for func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }() < int64(500) {
		{ v, _ := stack_i64.PeekAt(0); stack_input.Push(v) } // push k
	...
	stack_i64.PushAt(1, intToBytes(int64(0))) // var doubled
	{ // for @input
		...
		for _forIdx := 0; _forIdx < _forLen; _forIdx++ {
			...
			{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(1, v) } // doubled = ...
			{ v, _ := stack_i64.PeekAt(1); stack_mapped.Push(v) } // push doubled
		}
	}
