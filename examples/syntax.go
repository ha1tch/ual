...
func main() {
	...
	{ b, _ := stack_calc.Pop(); a, _ := stack_calc.Pop(); stack_calc.Push(intToBytes(bytesToInt(a) * bytesToInt(b))) }
	result := func() int64 { v, _ := stack_calc.Pop(); return bytesToInt(v) }()
	stack_numbers = ual.NewStack(ual.LIFO, ual.TypeInt64)
	stack_numbers.Push(intToBytes(100))
	stack_numbers.Push(intToBytes(200))
	stack_numbers.Push(intToBytes(300))
