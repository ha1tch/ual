...
func main() {
	stack_numbers := ual.NewStack(ual.LIFO, ual.TypeInt64)
	stack_numbers.Push(intToBytes(10))
	stack_numbers.Push(intToBytes(20))
	stack_numbers.Push(intToBytes(30))
	stack_numbers.Push(intToBytes(40))
	stack_numbers.Push(intToBytes(50))
	{ // for @numbers
		_forLen := stack_numbers.Len()
		for _forIdx := _forLen - 1; _forIdx >= 0; _forIdx-- {
			_forVal, _ := stack_numbers.PeekAt(_forIdx)
			stack_dstack.Push(_forVal)
	...
	{ // for @numbers
		...
		for _forIdx := _forLen - 1; _forIdx >= 0; _forIdx-- {
			_forVal, _ := stack_numbers.PeekAt(_forIdx)
			stack_i64.PushAt(0, _forVal) // v
			{ v, _ := stack_i64.PeekAt(0); stack_dstack.Push(v) } // push v
			{ v, _ := stack_i64.PeekAt(0); stack_dstack.Push(v) } // push v
	...
	{ // for @numbers
		...
		for _forIdx := 0; _forIdx < _forLen; _forIdx++ {
			_forVal, _ := stack_numbers.PeekAt(_forIdx)
			stack_i64.PushAt(1, intToBytes(int64(_forIdx))) // i
			stack_i64.PushAt(2, _forVal) // v
			{ v, _ := stack_i64.PeekAt(1); stack_dstack.Push(v) } // push i
			{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
			{ v, _ := stack_i64.PeekAt(2); stack_dstack.Push(v) } // push v
			{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
		}
	}
	stack_i64.PushAt(3, intToBytes(int64(0))) // var sum
	{ // for @numbers
		_forLen := stack_numbers.Len()
		for _forIdx := _forLen - 1; _forIdx >= 0; _forIdx-- {
			_forVal, _ := stack_numbers.PeekAt(_forIdx)
			stack_i64.PushAt(4, _forVal) // v
			{ v, _ := stack_i64.PeekAt(3); stack_dstack.Push(v) } // push sum
			{ v, _ := stack_i64.PeekAt(4); stack_dstack.Push(v) } // push v
			{ b, _ := stack_dstack.Pop(); a, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(a) + bytesToInt(b))) }
			{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(3, v) } // sum = ...
		}
	}
	{ v, _ := stack_i64.PeekAt(3); stack_dstack.Push(v) } // push sum
	{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
	stack_i64.PushAt(5, intToBytes(int64(0))) // var count
	{ // for @numbers
		_forLen := stack_numbers.Len()
		for _forIdx := _forLen - 1; _forIdx >= 0; _forIdx-- {
			_forVal, _ := stack_numbers.PeekAt(_forIdx)
			stack_i64.PushAt(6, _forVal) // v
			{ v, _ := stack_i64.PeekAt(5); stack_dstack.Push(v) } // push count
			{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) + 1)) }
			{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(5, v) } // count = ...
		}
	}
	{ v, _ := stack_i64.PeekAt(5); stack_dstack.Push(v) } // push count
	{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
