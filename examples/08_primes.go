...
func main() {
	stack_i64.PushAt(0, intToBytes(int64(0))) // var primeCount
	stack_i64.PushAt(1, intToBytes(int64(2))) // var num
	for func() int64 { v, _ := stack_i64.PeekAt(1); return bytesToInt(v) }() <= int64(500) {
		stack_i64.PushAt(2, intToBytes(int64(1))) // var isPrime
		stack_i64.PushAt(3, intToBytes(int64(2))) // var divisor
		for func() int64 { v, _ := stack_i64.PeekAt(3); return bytesToInt(v) }() < func() int64 { v, _ := stack_i64.PeekAt(1); return bytesToInt(v) }() {
			{ v, _ := stack_i64.PeekAt(1); stack_dstack.Push(v) } // push num
			{ v, _ := stack_i64.PeekAt(3); stack_dstack.Push(v) } // push divisor
			{ b, _ := stack_dstack.Pop(); a, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(a) % bytesToInt(b))) }
			{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(4, v) } // let remainder
			if func() int64 { v, _ := stack_i64.PeekAt(4); return bytesToInt(v) }() == int64(0) {
				stack_dstack.Push(intToBytes(0))
				{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(2, v) } // isPrime = ...
				break
			}
			{ v, _ := stack_i64.PeekAt(3); stack_dstack.Push(v) } // push divisor
			{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) + 1)) }
			{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(3, v) } // divisor = ...
		}
		if func() int64 { v, _ := stack_i64.PeekAt(2); return bytesToInt(v) }() > int64(0) {
			{ v, _ := stack_i64.PeekAt(0); stack_dstack.Push(v) } // push primeCount
			{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) + 1)) }
			{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(0, v) } // primeCount = ...
		}
		{ v, _ := stack_i64.PeekAt(1); stack_dstack.Push(v) } // push num
		{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) + 1)) }
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(1, v) } // num = ...
	}
	{ v, _ := stack_i64.PeekAt(0); stack_dstack.Push(v) } // push primeCount
