...
func main() {
	stack_i64.PushAt(0, intToBytes(int64(10))) // var x
	if func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }() > int64(5) {
		stack_dstack.Push(intToBytes(100))
		{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
	}
	stack_i64.PushAt(1, intToBytes(int64(3))) // var y
	if func() int64 { v, _ := stack_i64.PeekAt(1); return bytesToInt(v) }() > int64(5) {
		stack_dstack.Push(intToBytes(200))
		{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
	} else {
		stack_dstack.Push(intToBytes(300))
		{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
	}
	stack_i64.PushAt(2, intToBytes(int64(50))) // var z
	if func() int64 { v, _ := stack_i64.PeekAt(2); return bytesToInt(v) }() > int64(100) {
		stack_dstack.Push(intToBytes(1))
		{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
	} else if func() int64 { v, _ := stack_i64.PeekAt(2); return bytesToInt(v) }() > int64(50) {
	...
	stack_i64.PushAt(3, intToBytes(int64(5))) // var count
	for func() int64 { v, _ := stack_i64.PeekAt(3); return bytesToInt(v) }() > int64(0) {
	...
	stack_i64.PushAt(4, intToBytes(int64(3))) // var n
	for func() int64 { v, _ := stack_i64.PeekAt(4); return bytesToInt(v) }() > int64(0) {
		if func() int64 { v, _ := stack_i64.PeekAt(4); return bytesToInt(v) }() == int64(2) {
			stack_dstack.Push(intToBytes(999))
			{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
		} else {
			{ v, _ := stack_i64.PeekAt(4); stack_dstack.Push(v) } // push n
			{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
		}
		{ v, _ := stack_i64.PeekAt(4); stack_dstack.Push(v) } // push n
		{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) - 1)) }
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(4, v) } // n = ...
	}
	stack_i64.PushAt(5, intToBytes(int64(10))) // var i
	for func() int64 { v, _ := stack_i64.PeekAt(5); return bytesToInt(v) }() > int64(0) {
		{ v, _ := stack_i64.PeekAt(5); stack_dstack.Push(v) } // push i
		{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
		if func() int64 { v, _ := stack_i64.PeekAt(5); return bytesToInt(v) }() == int64(7) {
			break
		}
		{ v, _ := stack_i64.PeekAt(5); stack_dstack.Push(v) } // push i
		{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) - 1)) }
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(5, v) } // i = ...
	}
	stack_i64.PushAt(6, intToBytes(int64(5))) // var j
	for func() int64 { v, _ := stack_i64.PeekAt(6); return bytesToInt(v) }() > int64(0) {
		{ v, _ := stack_i64.PeekAt(6); stack_dstack.Push(v) } // push j
		{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) - 1)) }
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(6, v) } // j = ...
		if func() int64 { v, _ := stack_i64.PeekAt(6); return bytesToInt(v) }() == int64(2) {
			continue
		}
		{ v, _ := stack_i64.PeekAt(6); stack_dstack.Push(v) } // push j
		{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
