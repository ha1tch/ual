...
func main() {
	stack_state := ual.NewStack(ual.Hash, ual.TypeInt64)
	stack_state.Push(intToBytes(42), []byte("count")) // set "count"
	stack_state.Push(intToBytes(100), []byte("limit")) // set "limit"
	{ v, err := stack_state.Peek([]byte("count")); if err != nil { panic(err) }; stack_dstack.Push(v) } // get "count"
	{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
	{ v, err := stack_state.Peek([]byte("limit")); if err != nil { panic(err) }; stack_dstack.Push(v) } // get "limit"
	{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
