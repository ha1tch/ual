...
func main() {
	stack_result := ual.NewStack(ual.LIFO, ual.TypeInt64)
	stack_result.Push(intToBytes(5))
	func() {
		stack_result.Lock()
		defer stack_result.Unlock()
		_bytes_start, _err_start := stack_result.PopRaw()
		if _err_start != nil { panic(_err_start) }
		var start int64 = bytesToInt(_bytes_start)
		var queue [64]int64
		var visited [64]int64
		var head int64 = 0
		var tail int64 = 0
		var count int64 = 0
		queue[int(tail)] = start
		tail = (tail + 1)
		visited[int(start)] = 1
		for (head < tail) {
			var u int64 = queue[int(head)]
			head = (head + 1)
			count = (count + 1)
			var n1 int64 = (u - 1)
			if (n1 >= 0) {
		...
		stack_result.PushRaw(intToBytes(count))
		return
	}()
	{ v, _ := stack_result.Pop(); fmt.Println(bytesToInt(v)) }
