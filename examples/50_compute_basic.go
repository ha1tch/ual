...
func main() {
	stack_math := ual.NewStack(ual.LIFO, ual.TypeInt64)
	stack_math.Push(intToBytes(10))
	stack_math.Push(intToBytes(20))
	func() {
		stack_math.Lock()
		defer stack_math.Unlock()
		_bytes_a, _err_a := stack_math.PopRaw()
		if _err_a != nil { panic(_err_a) }
		var a int64 = bytesToInt(_bytes_a)
		_bytes_b, _err_b := stack_math.PopRaw()
		if _err_b != nil { panic(_err_b) }
		var b int64 = bytesToInt(_bytes_b)
		var result int64 = (a * b)
		stack_math.PushRaw(intToBytes(result))
		return
	}()
	{ v, _ := stack_math.Pop(); stack_dstack.Push(v) }
