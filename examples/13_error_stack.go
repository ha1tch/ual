...
func main() {
	stack_error.Push([]byte("first error"))
	stack_error.Push([]byte("second error"))
	stack_bool.Push(boolToBytes(stack_error.Len() > 0))
	stack_i64.PushAt(0, intToBytes(int64(0))) // var result
	stack_i64.PushAt(1, intToBytes(int64(100))) // var numerator
	stack_i64.PushAt(2, intToBytes(int64(0))) // var denominator
	if func() int64 { v, _ := stack_i64.PeekAt(2); return bytesToInt(v) }() == int64(0) {
		...
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(0, v) } // result = ...
	} else {
		{ v, _ := stack_i64.PeekAt(1); stack_dstack.Push(v) } // push numerator
		{ v, _ := stack_i64.PeekAt(2); stack_dstack.Push(v) } // push denominator
	...
	{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(2, v) } // denominator = ...
	if func() int64 { v, _ := stack_i64.PeekAt(2); return bytesToInt(v) }() == int64(0) {
		stack_error.Push([]byte("division by zero"))
		stack_dstack.Push(intToBytes(0))
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(0, v) } // result = ...
	...
	stack_error.Push([]byte("error A"))
	stack_error.Push([]byte("error B"))
	stack_error.Push([]byte("error C"))
	for stack_error.Len() > 0 { stack_error.Pop() }
