...
func main() {
	...
	for func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }() < int64(1000) {
		{ v, _ := stack_i64.PeekAt(0); stack_haystack.Push(v) } // push k
	...
	stack_i64.PushAt(1, intToBytes(int64(777))) // var needle
	stack_i64.PushAt(2, intToBytes(int64(0))) // var found
	stack_i64.PushAt(3, intToBytes(int64(0))) // var position
	{ // for @haystack
		_forLen := stack_haystack.Len()
		for _forIdx := 0; _forIdx < _forLen; _forIdx++ {
			_forVal, _ := stack_haystack.PeekAt(_forIdx)
			stack_i64.PushAt(4, intToBytes(int64(_forIdx))) // idx
			stack_i64.PushAt(5, _forVal) // val
			if func() int64 { v, _ := stack_i64.PeekAt(5); return bytesToInt(v) }() == func() int64 { v, _ := stack_i64.PeekAt(1); return bytesToInt(v) }() {
				stack_dstack.Push(intToBytes(1))
				{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(2, v) } // found = ...
				{ v, _ := stack_i64.PeekAt(4); stack_dstack.Push(v) } // push idx
				{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(3, v) } // position = ...
			}
		}
	}
	if func() int64 { v, _ := stack_i64.PeekAt(2); return bytesToInt(v) }() > int64(0) {
		{ v, _ := stack_i64.PeekAt(3); stack_dstack.Push(v) } // push position
