...
func main() {
	stack_i64.PushAt(0, intToBytes(int64(12))) // var num
	stack_i64.PushAt(1, intToBytes(int64(1))) // var fact
	stack_i64.PushAt(2, intToBytes(int64(1))) // var j
	for func() int64 { v, _ := stack_i64.PeekAt(2); return bytesToInt(v) }() <= func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }() {
		{ v, _ := stack_i64.PeekAt(1); stack_dstack.Push(v) } // push fact
		{ v, _ := stack_i64.PeekAt(2); stack_dstack.Push(v) } // push j
		{ b, _ := stack_dstack.Pop(); a, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(a) * bytesToInt(b))) }
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(1, v) } // fact = ...
		{ v, _ := stack_i64.PeekAt(2); stack_dstack.Push(v) } // push j
		{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) + 1)) }
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(2, v) } // j = ...
	}
	{ v, _ := stack_i64.PeekAt(1); stack_dstack.Push(v) } // push fact
