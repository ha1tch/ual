...
func main() {
	stack_i64.PushAt(0, intToBytes(int64(0))) // var totalPow
	stack_i64.PushAt(1, intToBytes(int64(1))) // var i
	for func() int64 { v, _ := stack_i64.PeekAt(1); return bytesToInt(v) }() <= int64(100) {
		stack_i64.PushAt(2, intToBytes(int64(2))) // var base
		stack_i64.PushAt(3, intToBytes(int64(0))) // var exp
		{ v, _ := stack_i64.PeekAt(1); stack_dstack.Push(v) } // push i
		stack_dstack.Push(intToBytes(20))
		{ b, _ := stack_dstack.Pop(); a, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(a) % bytesToInt(b))) }
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(3, v) } // exp = ...
		stack_i64.PushAt(4, intToBytes(int64(1))) // var result
		for func() int64 { v, _ := stack_i64.PeekAt(3); return bytesToInt(v) }() > int64(0) {
			{ v, _ := stack_i64.PeekAt(4); stack_dstack.Push(v) } // push result
			{ v, _ := stack_i64.PeekAt(2); stack_dstack.Push(v) } // push base
			{ b, _ := stack_dstack.Pop(); a, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(a) * bytesToInt(b))) }
			{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(4, v) } // result = ...
			{ v, _ := stack_i64.PeekAt(3); stack_dstack.Push(v) } // push exp
			{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) - 1)) }
			{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(3, v) } // exp = ...
		}
		{ v, _ := stack_i64.PeekAt(0); stack_dstack.Push(v) } // push totalPow
		{ v, _ := stack_i64.PeekAt(4); stack_dstack.Push(v) } // push result
		{ b, _ := stack_dstack.Pop(); a, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(a) + bytesToInt(b))) }
		{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(0, v) } // totalPow = ...
		{ v, _ := stack_i64.PeekAt(1); stack_dstack.Push(v) } // push i
		{ v, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(v) + 1)) }
