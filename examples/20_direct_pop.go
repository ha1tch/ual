...
func main() {
	...
	stack_inbox.Push(intToBytes(100))
	stack_inbox.Push(intToBytes(200))
	stack_inbox.Push(intToBytes(300))
	stack_i64.PushAt(0, intToBytes(int64(0))) // var msg
	{ v, _ := stack_inbox.Pop(); stack_i64.PushAt(0, v) } // msg = pop
	{ v, _ := stack_i64.PeekAt(0); stack_dstack.Push(v) } // push msg
	...
	{ v, _ := stack_i64.PeekAt(0); stack_dstack.Push(v) } // push msg
	{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
	stack_jobs := ual.NewStack(ual.LIFO, ual.TypeInt64)
	spawn_mu.Lock()
	spawn_tasks = append(spawn_tasks, func() {
		stack_i64.PushAt(1, intToBytes(int64(0))) // var i
		for func() int64 { v, _ := stack_i64.PeekAt(1); return bytesToInt(v) }() < int64(10000) {
	...
	stack_i64.PushAt(2, intToBytes(int64(0))) // var task
	{ v, _ := stack_jobs.Take(); stack_i64.PushAt(2, v) } // task = take
	{ v, _ := stack_i64.PeekAt(2); stack_dstack.Push(v) } // push task
	{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
