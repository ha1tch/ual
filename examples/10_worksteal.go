...
func main() {
	...
	stack_i64.PushAt(0, intToBytes(int64(0))) // var i
	for func() int64 { v, _ := stack_i64.PeekAt(0); return bytesToInt(v) }() < int64(100) {
		{ v, _ := stack_i64.PeekAt(0); stack_tasks.Push(v) } // push i
	...
	stack_i64.PushAt(1, intToBytes(int64(0))) // var ownerWork
	stack_i64.PushAt(2, intToBytes(int64(0))) // var thiefWork
	...
	{ // for @tasks
		...
		for _forIdx := _forLen - 1; _forIdx >= 0; _forIdx-- {
			...
			if func() int64 { v, _ := stack_i64.PeekAt(3); return bytesToInt(v) }() < int64(60) {
				{ v, _ := stack_i64.PeekAt(1); stack_dstack.Push(v) } // push ownerWork
				...
				{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(1, v) } // ownerWork = ...
			} else {
				{ v, _ := stack_i64.PeekAt(2); stack_dstack.Push(v) } // push thiefWork
				{ v, _ := stack_i64.PeekAt(4); stack_dstack.Push(v) } // push v
				{ b, _ := stack_dstack.Pop(); a, _ := stack_dstack.Pop(); stack_dstack.Push(intToBytes(bytesToInt(a) + bytesToInt(b))) }
				{ v, _ := stack_dstack.Pop(); stack_i64.PushAt(2, v) } // thiefWork = ...
			}
