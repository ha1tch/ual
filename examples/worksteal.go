...
func main() {
	stack_tasks := ual.NewCappedStack(ual.LIFO, ual.TypeInt64, 100)
	stack_tasks.Push(intToBytes(1))
	stack_tasks.Push(intToBytes(2))
	stack_tasks.Push(intToBytes(3))
	stack_tasks.Push(intToBytes(4))
	stack_tasks.Push(intToBytes(5))
	view_owner := ual.NewView(ual.LIFO)
	view_thief := ual.NewView(ual.FIFO)
	view_owner.Attach(stack_tasks)
	view_thief.Attach(stack_tasks)
	ownerTask := func() int64 { v, _ := view_owner.Pop(); return bytesToInt(v) }()
	thiefTask := func() int64 { v, _ := view_thief.Pop(); return bytesToInt(v) }()
	...
	sum := bytesToInt(ual.Reduce(stack_numbers, intToBytes(0), func(acc, elem []byte) []byte { a := bytesToInt(acc); b := bytesToInt(elem); return intToBytes((a + b)) }))
	
	// Results
	fmt.Printf("ownerTask = %v\n", ownerTask)
	fmt.Printf("thiefTask = %v\n", thiefTask)
	fmt.Printf("sum = %v\n", sum)
