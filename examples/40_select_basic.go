...
func main() {
	...
	func() {
		// setup
		stack_inbox.Push(intToBytes(1))
		{ v, _ := stack_inbox.Pop(); fmt.Println(bytesToInt(v)) }
		
		// Non-blocking select: check stacks in order
		if stack_inbox.Len() > 0 {
			_v, _ := stack_inbox.Pop()
			msg := bytesToInt(_v)
			_ = msg // suppress unused warning
			stack_dstack.Push(intToBytes(msg))
			{ v, _ := stack_dstack.Pop(); fmt.Println(bytesToInt(v)) }
		} else if stack_commands.Len() > 0 {
			_v, _ := stack_commands.Pop()
			cmd := bytesToInt(_v)
