...
ual-generated compute blocks achieve **near-native Go performance**. Since ual compiles to Go (or Rust), the compute block code runs at the same speed as idiomatic compiled code. This comparison includes C and Python to show where ual sits in the performance spectrum.

**Note:** The Rust backend (`--target rust`) produces identical output to Go with comparable performance. Binary sizes differ significantly — see `docs/BENCHMARKS.md` for binary size comparison.
